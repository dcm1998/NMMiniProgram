<template>
	<view class="lonjin-form-wrapper">
		...
		<view class="items flex-bt" :class="[getClassName(item)]" v-for="(item, index) in config" :key="index">
			...
			<view class="value flex-end">
				<template v-if="item.type == 'input'">
					<input :type="item.inputType" :value="form[item.key]" @input="change" :data-key="item.key"
				...
				<template v-if="item.type == 'picker'">
					<view class="picker flex-end">
						<picker @change="pickerChange" range-key="label" :data-options="item.options"
							:data-key="item.key" :value="findIndex(item.options, item.key)" :range="item.options">
							<view class="uni-input" :class="[getLabel(item.options, item.key) ? '' : 'placeholder']">{{
								getLabel(item.options, item.key) ? getLabel(item.options, item.key) : item.placeholder}}
							</view>
						</picker>
				...
				<template v-if="item.type == 'multipleChoice'">
					<view class="options flex-start">
						<view class="options-item flex-start" @click="chooseItem(item.key, child.value)"
							v-for="(child, childIndex) in item.options" :key="childIndex">
				...
				<template v-if="item.type == 'uploadMedia'">
					<view class="upload flex-end" @click="upload(item)">
						<template v-if="form[item.key]">
							已上传
							<image class="icon" src="../../static/images/add-icon-3.png"></image>
						</template>
						<template v-else>
							请上传
							<image class="icon" src="../../static/images/add-icon-3.png"></image>
				...
				<template v-if="item.type == 'date' || item.type == 'time'">
					<view class="picker flex-end">
						<picker :start="item.start" :end="item.end" :fields="item.fields || 'day'" @change="dateChange"
							:mode="item.type" :data-key="item.key" :value="form[item.key]">
							<view class="uni-input" :class="[form[item.key] ? '' : 'placeholder']">{{ form[item.key] ?
								form[item.key] : item.placeholder}}</view>
						</picker>
						<!-- <image class="icon" src="../../static/images/icon-right.png"></image> -->
					</view>
				</template>
  ...
  <view class="picker flex-end">
    <picker 
      ...
      :range="item.range"
    >
      <view class="uni-input" :class="{ placeholder: !form[item.key] }">
    ...
    <image class="icon" src="../../static/images/icon-right.png"></image>
				...
				<template v-if="item.type == 'textarea'">
					<textarea :value="form[item.key]" @input="change" :data-key="item.key"
						:placeholder="item.placeholder" cols="30" rows="10"></textarea>
				</template>
				<template v-if="item.type == 'pictureGroup'">
					<view class="pictureGroup flex-start">
						<view class="child flex-center" v-for="(child, childIndex) in form[item.key]" :key="childIndex">
							...
							<image @click="deleteImage(item, child)" src="../../static/images/close.png" class="close">
							</image>
						</view>
						<view v-if="!form[item.key] || (form[item.key].length) < (item.max || 1)"
							class="add flex-center" @click="uploadPicture(item)">
							<image src="../../static/images/add-icon-3.png" mode=""></image>
							<view>请上传</view>
						</view>
					</view>
				</template>
...
//   // uniapp 的 time picker 返回的是 HH:mm 格式字符串
//   this.form[key] = value;
//   this.$emit('updateValue', this.form);
// }
